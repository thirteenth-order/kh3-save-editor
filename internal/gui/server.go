...
func (s *Server) guard(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Gate 2: the token, from a header, the query string, or the path
		// segment an asset request carries. A tag cannot set a header, and a
		// module import cannot pass on a query string, so the value travels by
		// whichever transport the thing asking for it supports. The comparison
		// below is the same one in all three cases.
		got := r.Header.Get(tokenHeader)
		if got == "" {
			got = r.URL.Query().Get("t")
		}
		if got == "" {
			got, _, _ = assetName(r.URL.Path)
		}
...
// assetPrefix is where every asset but the page itself is served from, with
// the run's token as a path segment.
//
// The token has to be in the path rather than the query string because the
// page's scripts are ES modules: a relative import resolves against the
// importing module's URL and does not inherit its query, so `import "./ui.js"`
// from /app.js?t=... would ask for a bare /ui.js and be refused. A path
// segment is inherited, so one substitution in index.html carries the token
// to every module, stylesheet and image the page pulls in.
//
// It is the same token and the same four gates. Nothing is relaxed here: the
// guard still compares it in constant time, and it is only ever read out of a
// path shaped exactly like an asset request.
const assetPrefix = "/a/"
...
// assetName splits an asset request into its token and its file name. It
// answers only for a path of exactly the form /a/<token>/<name> where <name>
// is on the allow-list, so a traversal, a nested path or an unknown file is
// not an asset request at all and gets no token out of this.
func assetName(path string) (token, name string, ok bool) {
	rest, found := strings.CutPrefix(path, assetPrefix)
	if !found {
		return "", "", false
	}
	tok, file, found := strings.Cut(rest, "/")
	if !found || tok == "" {
		return "", "", false
	}
	if _, listed := staticFiles[file]; !listed {
		return "", "", false
	}
...
func (s *Server) handleIndex(w http.ResponseWriter, r *http.Request) {
	name := ""
	switch _, file, ok := assetName(r.URL.Path); {
	case ok:
		name = file
	case r.URL.Path == "/":
		name = "index.html"
	...
	// subresource URLs carry it in their path instead, and one substitution
	// here is what puts it there. Without this the guard answers 403 and the
	// browser drops the stylesheet on its MIME type.
	if name == "index.html" {
